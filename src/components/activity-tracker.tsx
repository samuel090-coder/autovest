...
/** Silently records page views + session start (with IP/geo) for signed-in users. */
...
export function trackEvent(event: string, label?: string, meta?: Record<string, unknown>) {
  void logActivity({
    data: { event, label, path: typeof window !== "undefined" ? window.location.pathname : undefined, meta },
  }).catch(() => {});
}
