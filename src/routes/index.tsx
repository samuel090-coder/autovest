...
function Home() {
  ...
  const welfare = banners.find((b) => b.key === "welfare_hero");

  const { data: hero } = useQuery({
    queryKey: ["hero-investment"],
    queryFn: async () => {
  ...
  return (
    <AppShell>
      ...
      <div className="px-4 pt-4">
        <div className="bg-balance-card relative overflow-hidden rounded-2xl p-5 text-white shadow-lg">
          <div className="flex items-center justify-between">
            ...
            <Link to="/wallet" className="rounded-md bg-info px-3 py-1.5 text-xs font-semibold inline-flex items-center gap-1">
        ...
        {actions.map(({ label, icon: Icon, to, badge }) => (
          <Link key={label} to={to} className="flex flex-col items-center gap-1.5">
            <div className="relative grid h-12 w-12 place-items-center rounded-full bg-card shadow-sm ring-1 ring-border">
              ...
              {badge ? (
                <span className="absolute -right-1 -top-1 grid h-5 min-w-5 place-items-center rounded-full bg-brand px-1 text-[10px] font-bold text-white">{badge}</span>
              ) : null}
            </div>
      ...
      <div className="px-4 pt-5">
        <Link to={lucky?.link ?? "/wallet"} className="block">
          <div className="bg-lucky-gradient relative h-28 overflow-hidden rounded-2xl p-5 text-white shadow-md">
            {lucky?.image_url && (
              <img src={lucky.image_url} alt="" className="absolute inset-0 h-full w-full object-cover opacity-60" />
            )}
      ...
      {/* Welfare Product hero */}
      <div className="px-4 pt-5">
        <div className="bg-welfare-gradient rounded-2xl p-3">
          <div className="py-1 text-center text-lg font-bold text-white">{welfare?.title ?? "Welfare Product"}</div>
          {hero && (
            <Link to="/investment/$id" params={{ id: hero.id }} className="block">
              <div className="overflow-hidden rounded-xl bg-card">
                <div className="relative aspect-[16/10] w-full bg-muted">
                  {hero.image_url ? (
                    <img src={hero.image_url} alt={hero.name} className="h-full w-full object-cover" />
                  ) : null}
                  {hero.is_hot && (
                    <span className="absolute left-3 top-3 rounded-full bg-brand px-2 py-0.5 text-[10px] font-bold text-white">HOT</span>
                  )}
                </div>
                <div className="p-4">
                  <div className="text-base font-bold">{hero.name}</div>
                  <div className="mt-3 grid grid-cols-3 gap-2 text-center">
                    <Stat label="Days" value={String(hero.cycle_days)} highlight="dark" sub="Investment Cycle" />
                    <Stat label="Daily Income" value={Number(hero.daily_income).toLocaleString()} highlight="info" />
                    <Stat label="Total Income" value={Number(hero.total_income).toLocaleString()} highlight="info" />
                  </div>
                  <Button className="mt-3 w-full bg-info text-white hover:bg-info/90 text-base">
      ...
      <div className="px-4 pt-6 pb-2">
        <h2 className="text-lg font-bold">Product Center</h2>
      </div>
      <ProductList />
...
function Stat({ label, value, sub, highlight }: { label: string; value: string; sub?: string; highlight: "dark" | "info" }) {
  return (
    <div>
      <div className={highlight === "info" ? "text-info font-bold" : "font-bold"}>{value}</div>
      <div className="text-[11px] text-muted-foreground">{sub ?? label}</div>
    </div>
  );
}
  ...
  if (items.length === 0) {
    return (
      <div className="px-4 pb-6 text-center text-sm text-muted-foreground">
        No products yet. Admins can add them from the admin panel.
      </div>
    );
  }
  return (
        ...
        <Link key={p.id} to="/investment/$id" params={{ id: p.id }} className="block">
          <div className="rounded-2xl bg-card p-4 shadow-sm">
            <div className="flex items-start gap-3">
              <div className="flex-1">
                ...
                <div className="mt-1 text-sm">
                  ...
                  <span className="font-bold">{p.cycle_days}</span>
                </div>
              </div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-3 rounded-lg bg-muted px-4 py-3 text-center">
              <div>
                <div className="text-brand font-bold">{Number(p.daily_income).toLocaleString()}</div>
                <div className="text-xs">Daily Income</div>
              </div>
              <div>
                <div className="text-brand font-bold">{Number(p.total_income).toLocaleString()}</div>
            ...
            <div className="mt-3 flex items-center justify-between">
              <div>
                ...
                <span className="text-brand font-bold">{Number(p.price).toLocaleString()}</span>
              </div>
